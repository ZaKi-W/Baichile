...
const auth=useAuthStore(),loading=ref(false),error=ref(''),items=ref<AdminUser[]>([]),total=ref(0),dialog=ref(false),creating=ref(false);
...
async function load(){loading.value=true;error.value='';try{const r=await adminApi.listAdminUsers(filters);items.value=r.items;total.value=r.total;}catch(e){items.value=[];total.value=0;error.value=e instanceof Error?e.message:'管理员加载失败';}finally{loading.value=false;}}
