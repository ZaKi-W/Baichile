...
const route=useRoute(),auth=useAuthStore(),loading=ref(false),order=ref<(OrderRecord&{account?:AccountRecord;store?:StoreRecord})|null>(null),editOpen=ref(false);
...
<template>
  <div v-loading="loading"><div class="page-toolbar"><el-button @click="$router.back()">返回</el-button><span class="grow"/><el-button v-if="auth.has('orders:write')" type="primary" @click="openEdit">更新跟进状态</el-button></div>
    <template v-if="order"><div class="surface detail-grid"><div class="detail-item"><span>虚拟订单号</span><strong>{{order.id}}</strong></div><div class="detail-item"><span>用户</span><strong>{{order.account?.nickname||order.accountId||'游客'}}</strong></div><div class="detail-item"><span>商家</span><strong>{{order.store?.name||order.storeId}}</strong></div><div class="detail-item"><span>订单虚拟金额</span><strong>虚拟 ¥{{centsToYuan(order.totalCents)}}</strong></div><div class="detail-item"><span>虚拟配送状态</span><strong>{{order.status}}</strong></div><div class="detail-item"><span>后台状态</span><strong>{{order.adminStatus}}</strong></div><div class="detail-item"><span>创建时间</span><strong>{{formatDate(order.createdAt)}}</strong></div><div class="detail-item"><span>虚拟配送异常</span><strong>{{order.incidentKey||'无'}}</strong></div><div class="detail-item"><span>虚拟退款</span><strong>{{order.refundedAt?formatDate(order.refundedAt):'无'}}</strong></div></div>
