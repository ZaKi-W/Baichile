...
const EGGS = [
  ...
  ['coupon-scholar', '满减理论博士', 'rare', '算出的不只是优惠，还有一次复杂推演。', '#D97706', 'calculator'],
...
const PERSONAS: SharePersona[] = [
  ...
  persona('caln', 'CALN', '模拟热量忍者', '模拟热量刚到门口，你已经完成了整桌推演。', '对模拟热量极其敏锐，最享受在虚拟点单里研究一整桌。', '看看你的模拟热量人格'),
  persona('cpns', 'CPNS', '满减策略学者', '不一定要吃，但优惠必须算明白。', '你把虚拟点单当作数学研究，凑单、满减与配送费都逃不过你的演算。', '测测你的满减流派'),
  ...
  persona('budg', 'BUDG', '模拟订单金额审计官', '每一笔模拟订单金额，都要算得有凭有据。', '你对价格有天然雷达，享受看着累计模拟订单金额稳定上涨。', '测测你的模拟订单金额人格'),
...
export function selectOrderEasterEgg(
  orderId: string,
  seed: string,
  triggeredAt: string,
  successEggRate = 0.1,
): OrderEasterEgg | undefined {
  const hash = stableHash(`${orderId}:${seed}:egg`);
  const rate = Math.min(1, Math.max(0, successEggRate));
  if (hash % 10_000 >= Math.round(rate * 10_000)) return undefined;
