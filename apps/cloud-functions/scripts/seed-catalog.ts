...
async function main() {
  ...
  const counts = await seedCatalog(db, {
    clearExisting: process.env.CATALOG_SKIP_CLEAR !== 'true',
    includeBase: process.env.CATALOG_SKIP_BASE !== 'true',
    menuOffset: parseNonNegativeInteger(process.env.CATALOG_MENU_OFFSET, 0),
    menuLimit: process.env.CATALOG_MENU_LIMIT === undefined
      ? undefined
      : parseNonNegativeInteger(process.env.CATALOG_MENU_LIMIT, 0),
    writeBatchSize: parseNonNegativeInteger(process.env.CATALOG_WRITE_BATCH_SIZE, 10) || 10,
  });
...
export async function seedCatalog(
  ...
  if (includeBase) {
    await writeInBatches(categories.map((category, sortOrder) => ({ ...category, sortOrder })), writeBatchSize, (category) =>
    ...
    await writeInBatches(stores.map((store, sortOrder) => ({ ...store, sortOrder })), writeBatchSize, (store) => {
  ...
  if (includeBase) {
    await writeInBatches(subCategories, writeBatchSize, (row) =>
  ...
  const selectedMenuItems = menuItems.slice(menuOffset, menuLimit === undefined ? undefined : menuOffset + menuLimit);
  await writeInBatches(selectedMenuItems, writeBatchSize, (row) =>
    db.collection<MenuItemDoc>(collections.menuItems).upsert(row.id, row));

  return {
    categories: includeBase ? categories.length : 0,
    stores: includeBase ? stores.length : 0,
