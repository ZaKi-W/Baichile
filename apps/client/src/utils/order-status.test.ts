...
describe('order status presentation', () => {
  it('returns the current persisted timeline step from the order start time', () => {
    ...
    expect(getOrderStep(startedAt, deliveryDurationMs, startedAt + 18_000).label).toBe('骑手接单');
    expect(getOrderStep(startedAt, deliveryDurationMs, startedAt + 83_000).label).toBe('配送中');
    expect(getOrderStep(startedAt, deliveryDurationMs, startedAt + 83_000 + deliveryDurationMs - 1)).toBe(ORDER_STEPS[5]);
    expect(getOrderStep(startedAt, deliveryDurationMs, startedAt + 83_000 + deliveryDurationMs)).toBe(ORDER_STEPS[6]);
