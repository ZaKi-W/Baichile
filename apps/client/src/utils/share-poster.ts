...
export function buildSharePosterModel(data: ShareLanding, requestedKind?: SharePosterKind): SharePosterModel {
  ...
  if (kind === 'achievement') {
    return {
      ...
      secondary: `模拟订单金额 ¥${money(data.savedMoneyCents)} · 模拟热量 ${data.savedCaloriesKcal} 千卡`,
  ...
  if (kind === 'persona') {
    return {
      ...
      secondary: `${data.completedOrderCount} 顿模拟订单 · 模拟订单金额 ¥${money(data.savedMoneyCents)}`,
  ...
  return {
    ...
    secondary: `模拟热量 ${data.savedCaloriesKcal} 千卡`,
    detail: egg?.verdict || data.dishNames.slice(0, 3).join('、') || '一顿神秘空气外卖',
    accent: egg?.themeColor || '#FF7145', stamp: kind === 'order_egg' ? '彩蛋已解锁' : '模拟单完成',
    ticket: kind === 'order_egg' ? 'HIDDEN CAPSULE' : 'ORDER CAPSULE', primaryLabel: kind === 'order_egg' ? '本次获得' : '模拟订单金额', callToAction: kind === 'order_egg' ? '扫码抽取更多彩蛋' : '扫码开一单虚拟外卖',
    fundsNotice: VIRTUAL_FUNDS_NOTICE,
  };
}
